...
def dashboard(request):
    ...
    completed_quotes = [
        "Nice work — you just made this app jealous of your productivity!",
        "Well done, superstar — your task is complete and you look amazing doing it.",
        "You finished it like a boss — now go celebrate, you brilliant human!",
    ...
    if due_reminders.exists():
        for task in due_reminders:
            subject_name = task.subject.name if task.subject else 'your subject'
            if task.status == 'Pending':
                quote = random.choice(motivational_quotes)
                text = f"Pendinggg! Your task '{task.title}' for '{subject_name}' is still incomplete. {quote}"
            else:
                quote = random.choice(completed_quotes)
                text = f"Completed! Your task '{task.title}' in '{subject_name}' is done — great work! {quote}"
            reminder_notifications.append({'task_title': task.title, 'text': text})
        ...
        if pending_count > 0:
            ...
            reminder_notifications.append({
                'text': f"Pendinggg! You have {pending_count} incomplete task{'s' if pending_count != 1 else ''}. {quote}"
            })
